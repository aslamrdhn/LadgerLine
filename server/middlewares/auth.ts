 ...
 */
export async function enforceTenantBoundary(request: FastifyRequest, reply: FastifyReply) {
  ...
  if (!userProfile || userProfile.status !== 'active') {
    return reply.code(403).send({ error: 'FORBIDDEN', message: 'User inactive or not found' });
  }

  request.user.role = userProfile.role;
  request.user.assignedOutlets = userProfile.assignedOutlets ? JSON.parse(userProfile.assignedOutlets) : [];
}
 ...
 */
export function crossOutletValidation(outletId: string, assignedOutlets?: string[]): boolean {
