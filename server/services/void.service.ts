...
async function createVoidReversingJournal(
  ...
  const afterDiscount = new Decimal(sale.subtotal.toString()).minus(sale.discount ? sale.discount.toString() : 0);

  let lines = [
    { accountId: cashAccount.id, debit: new Decimal(0), credit: new Decimal(sale.totalAmount.toString()) },
    { accountId: revenueAccount.id, debit: afterDiscount, credit: new Decimal(0) },
    { accountId: hppAccount.id, debit: new Decimal(sale.totalHpp ? sale.totalHpp.toString() : 0), credit: new Decimal(0) },
    { accountId: inventoryAccount.id, debit: new Decimal(0), credit: new Decimal(sale.totalHpp ? sale.totalHpp.toString() : 0) },
    { accountId: taxPayableAccount.id, debit: new Decimal(sale.tax ? sale.tax.toString() : 0), credit: new Decimal(0) },
    { accountId: serviceChargeAccount.id, debit: new Decimal(sale.serviceCharge ? sale.serviceCharge.toString() : 0), credit: new Decimal(0) },
  ];
...
export async function voidFallback(input: {
  ...
  if (idem) {
    const parsed = typeof idem.responseBody === 'string' ? JSON.parse(idem.responseBody) : idem.responseBody;
    if (idem.responseBody === '{"processing": true}' || (parsed && parsed.processing)) {
