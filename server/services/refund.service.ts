...
export const RefundPayloadSchema = z.object({
  ...
  refundMethod: z.enum(['CASH', 'QRIS', 'DEBIT', 'CREDIT', 'BANK_TRANSFER']).default('CASH'),
});
...
export async function processRefund(input: TRefundPayload) {
  ...
  return await withTenant(validated.tenantId, async (tx: PrismaTransaction) => {
    ...
    for (const item of validated.items) {
      ...
      const totalDetailHpp = new Decimal(detail.hpp ? detail.hpp.toString() : 0);
    ...
    const totalDiscount = new Decimal(originalSale.discount ? originalSale.discount.toString() : 0);
    const totalTax = new Decimal(originalSale.tax ? originalSale.tax.toString() : 0);
    const totalService = new Decimal(originalSale.serviceCharge ? originalSale.serviceCharge.toString() : 0);
    ...
    for (const rd of refundItems) {
      ...
      const warehouseId = originalSale.warehouseId || originalSale.outlet?.defaultWarehouseId;
    ...
    let lines = [
      { accountId: revenueAccount.id, debit: roundedRefundSubtotal.minus(roundedRefundDiscount), credit: new Decimal(0) },
      { accountId: taxPayableAccount.id, debit: roundedRefundTax, credit: new Decimal(0) },
      { accountId: serviceChargeAccount.id, debit: roundedRefundService, credit: new Decimal(0) },
      { accountId: hppAccount.id, debit: new Decimal(0), credit: refundHpp },
      { accountId: inventoryAccount.id, debit: refundHpp, credit: new Decimal(0) },
    ...
    const kitchenRefundItems = await Promise.all(
      validated.items.map(async (item) => {
        const detail = await tx.salesDetail.findUnique({ where: { id: item.salesDetailId } });
...
export async function getRefundableItems(salesId: string, tenantId: string) {
  ...
  return sale.details.map((detail) => {
    ...
    const unitHpp = detail.quantity > 0 ? Number(detail.hpp) / detail.quantity : 0;
