...
export const financeRoutes: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  ...
  fastify.post('/export-drive', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { accessToken, rows, reportName } = request.body as any;
      if (!accessToken || !rows) {
        return reply.code(400).send({ success: false, message: 'Missing accessToken or rows' });
      }

      const result = await exportToGoogleSheets(accessToken, reportName || 'Laporan Keuangan', rows);
  ...
  fastify.post('/backup/restore', async (request: FastifyRequest, reply: FastifyReply) => {
    return reply.code(200).send({ success: true, message: 'Backup restored (mock)' });
  });
  ...
  fastify.get('/true-hpp', async (request, reply) => {
    try {
      ...
      const result = await financeService.getDashboardPerformance(tenantId, date);
  ...
  fastify.get('/profit-loss', async (request, reply) => {
    try {
      ...
      const result = await financeService.getProfitAndLoss(tenantId, startDate, endDate);
