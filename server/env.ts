...
if (!process.env.JWT_SECRET || process.env.JWT_SECRET.length < 32) {
  process.env.JWT_SECRET = 'ledgerline_super_secret_stable_key_for_production_fallback_32_chars';
  logger.warn('[ENV] JWT_SECRET is missing or too short. Using a fallback secret.');
}
...
const isInvalidDbUrl = !process.env.DATABASE_URL || 
  (!process.env.DATABASE_URL.startsWith('postgres://') && !process.env.DATABASE_URL.startsWith('postgresql://'));

if (isInvalidDbUrl) {
  logger.error('[ENV] DATABASE_URL is missing or invalid. It must start with postgresql:// or postgres://. Using dummy URL to allow server start, but database connection will fail.');
...
const envSchema = z.object({
  ...
  DEFAULT_OWNER_PASSWORD: z.string().min(1, 'DEFAULT_OWNER_PASSWORD is required'),
...
try {
  ...
  if (!env.BREVO_API_KEY) {
    logger.warn('[ENV] BREVO_API_KEY is not set. Email notifications will be skipped.');
  }
  ...
  if (process.env.NODE_ENV === 'test') {
    ...
    logger.error('[ENV] Server tidak bisa start karena konfigurasi environment tidak valid. Perbaiki variabel di atas lalu deploy ulang.');
    process.exit(1);
  }
}
