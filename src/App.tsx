...
export default function App() {
  ...
  const syncQuery = useAppSync(currentStore?.id, localStorage.getItem("ledgerline_jwt_token") || "");

  const isKasir = React.useMemo(() => currentStore?.cashierRole === 'Kasir', [currentStore]);
  ...
  if (!currentStore) {
    return (
      <div className="relative isolate min-h-screen">
        <StoreLoginPortal 
          tenants={[]} 
          onLogin={(t, tok) => { setCurrentStore(t); localStorage.setItem('aslam_ledger_current_store', JSON.stringify(t)); if (tok) localStorage.setItem('ledgerline_jwt_token', tok); }}
          onRegister={() => {}}
        />
      </div>
    );
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full">
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button onClick={() => setShowLogoutConfirm(false)} className="px-4 py-2 text-slate-500">Batal</button>
              <button onClick={executeLogout} className="px-4 py-2 bg-rose-600 text-white rounded-xl">Keluar Sesi</button>
            </div>
          </div>
        </div>
      )}
