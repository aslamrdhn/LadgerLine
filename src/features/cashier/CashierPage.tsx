      ...
      <div className="flex flex-wrap gap-2">
        ...
        {["All", "Coffee", "Snacks"].map((cat) => (
          <button
            ...
            className={`p-2 rounded ${category === cat ? "bg-blue-600 text-white" : "bg-gray-200"}`}
          >
            {cat}
          </button>
        ))}
  ...
  return (
    <div className="flex flex-col h-full p-4">
      ...
      <div className="border-t pt-4 mt-4">
        ...
        <div className="flex justify-between">
          <span>Pajak:</span> <span>Rp {tax.toLocaleString()}</span>
        </div>
        ...
        <button
          ...
          className="w-full bg-slate-900 text-white py-3 rounded mt-4"
        >
          Bayar Sekarang
        </button>
      </div>
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-sm text-center">
        ...
        {result.change > 0 && (
          <div className="mb-4 text-xl">
            Kembalian: Rp {result.change.toLocaleString()}
          </div>
        )}
        <button
          onClick={onClose}
          className="w-full px-4 py-2 bg-slate-900 text-white rounded"
        >
          Tutup & Lanjut
        </button>
      </div>
...
export default function CashierPage() {
  ...
  const handleCheckout = async () => {
    ...
    setCheckoutResult({
      success: true,
      order: newOrder,
      change: Number(cashGiven || 0) - total,
    });
