  ...
  Object.defineProperty(window, 'fetch', {
    ...
    value: async function (...args: any[]) {
      ...
      if (typeof resource === 'string' && resource.startsWith('/api/')) {
        ...
        if (response.status === 401 && resource.startsWith('/api/') && resource !== '/api/login' && resource !== '/api/refresh-token' && resource !== '/api/google-auth') {
          const refreshRes = await originalFetch('/api/refresh-token', { method: 'POST', credentials: 'include' });
          if (refreshRes.ok) {
