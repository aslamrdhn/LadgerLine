...
export function MainLayout({
  ...
  const theme = {
    slate: {
      ...
      activeTab: 'bg-blue-600/20 text-blue-450 border border-blue-500/20 font-extrabold shadow-sm',
  ...
  const menuItems = uiMode === 'simple' 
    ? [
        { id: 'tab-kasir', tab: 'kasir', name: 'Kasir', icon: Calculator },
        { id: 'tab-stok', tab: 'stok', name: 'Stok & Bahan', icon: Layers },
        ...(['TIER_2', 'TIER_3'].includes(appConfig?.subscriptionTier || 'TIER_1') ? [{ id: 'tab-meja', tab: 'meja', name: 'Order Meja', icon: QrCode }] : []),
        { id: 'tab-laporan', tab: 'laporan', name: 'Laporan', icon: FileBarChart2 },
               { id: 'tab-pengaturan', tab: 'pengaturan', name: 'Pengaturan', icon: Building2 },
             ]
    : [
        { id: 'tab-dashboard', tab: 'dashboard', name: 'Dashboard Overview', icon: LayoutDashboard },
        { id: 'tab-kasir', tab: 'kasir', name: 'Mesin Kasir POS', icon: Calculator },
  ...
  return (
    <div className={`min-h-screen ${theme.appBg} font-sans flex flex-col md:flex-row`} id="app-container">
      ...
      <aside className={`hidden md:flex w-20 lg:w-64 shrink-0 ${theme.asideBg} text-slate-300 flex-col border-r ${theme.border}`} id="sidebar">
        ...
        <div className="p-3 lg:p-4 flex-1 space-y-1.5 overflow-y-auto flex flex-col items-center lg:items-stretch">
          {demoTimeRemaining && (
            <div className="w-full p-2.5 bg-amber-500/10 border border-amber-500/30 rounded-xl text-center space-y-1 shadow-inner relative z-20">
              <p className="text-[9px] text-amber-500 font-extrabold uppercase tracking-widest leading-none">SESI DEMO AKTIF</p>
              <p className="text-[12px] text-white font-mono font-black animate-pulse leading-none">{demoTimeRemaining}</p>
            </div>
          )}
          ...
          {filteredMenu.map(({ id, tab, name, icon: Icon}) => (
            <button
              ...
              className={`w-full px-2 lg:px-3 py-3 rounded-xl justify-center lg:justify-start text-xs font-bold font-sans flex items-center gap-3 transition-all cursor-pointer ${
                activeTab === tab ? theme.activeTab : `${theme.inactiveTab} border border-transparent`
              }`}
              title={name}
            >
      ...
      <div className={`md:hidden fixed bottom-0 left-0 right-0 ${theme.asideBg} border-t ${theme.border} p-2 flex justify-around items-center z-40 pb-safe`}>
      ...
      <main className="flex-1 flex flex-col relative h-screen overflow-hidden pb-[60px] md:pb-0">
         ...
         <div className="fixed bottom-20 md:bottom-6 right-6 z-50 flex flex-col gap-2 pointer-events-none">
            {useUiStore(s => s.toasts).map((t) => (
              <div key={t.id} className="bg-slate-900 text-white px-5 py-3 rounded-xl shadow-xl flex items-center gap-3 border border-slate-700 animate-fade-in pointer-events-auto">
                <CheckCircle className="text-emerald-400 shrink-0" size={16} />
                <span className="text-xs font-bold font-sans">{t.message}</span>
                <button onClick={() => useUiStore.getState().removeToast(t.id)} className="ml-2 text-slate-400 hover:text-white">
                  <X size={14} />
                </button>
              </div>
            ))}
