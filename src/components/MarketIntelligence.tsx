...
export default function MarketIntelligence({
  ...
  const [supplierSubTab, setSupplierSubTab] = useState<"marketplace" | "mysuppliers">("marketplace");
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === "supply_network" && !loading && (
        <div className="space-y-6">
          <div className="flex gap-2 p-1 bg-slate-100 rounded-lg w-max border border-slate-200">
            <button onClick={() => setSupplierSubTab('marketplace')} className={`px-4 py-2 rounded-md font-bold text-sm ${supplierSubTab === 'marketplace' ? 'bg-white shadow text-slate-800' : 'text-slate-500 hover:text-slate-700'}`}>Marketplace Terverifikasi</button>
            <button onClick={() => setSupplierSubTab('mysuppliers')} className={`px-4 py-2 rounded-md font-bold text-sm ${supplierSubTab === 'mysuppliers' ? 'bg-white shadow text-slate-800' : 'text-slate-500 hover:text-slate-700'}`}>PO & Privat Supplier</button>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            <div className="mb-6">
              ...
              <p className="text-sm text-slate-500">
                Rekomendasi pemasok bahan baku berdasarkan data permintaan nyata
                ekosistem. Hubungi atau kirim Purchase Request (PR) langsung.
                Transaksi & negosiasi dilakukan di luar sistem.
              </p>
            </div>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
               <div className="space-y-4 border border-slate-200 bg-slate-50 p-6 rounded-2xl">
                  <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200">
                    <h3 className="font-sans font-bold text-sm text-slate-900">Daftar Supplier Privat</h3>
                    <button onClick={() => setShowAddMySupplier(true)} className="px-3 py-1.5 bg-indigo-600 text-white text-xs font-bold rounded-lg flex items-center gap-2 hover:bg-indigo-700">
                      <PlusCircle size={14}/> Tambah
                    </button>
                  </div>
                  <div className="space-y-3">
                     {mySuppliers.length === 0 ? (
                        <p className="text-xs text-slate-500 py-4 text-center bg-white rounded-xl border border-slate-200">Belum ada supplier privat terdaftar atau pesanan riwayat.</p>
                     ) : (
                       mySuppliers.map((s:any, idx:number) => (
                          <div key={idx} className="p-4 bg-white rounded-xl border border-slate-100 shadow-sm relative group">
                            <h4 className="text-sm font-black text-slate-800 flex items-center gap-2">{s.company_name} {s.verification_status === "VERIFIED" && (<CheckCircle2 size={16} className="text-blue-500" />)}</h4>
                            <p className="text-xs text-slate-500 mt-1">{s.address}</p>
                            <div className="mt-4 pt-4 border-t border-slate-100 flex flex-col gap-2">
                               {s.listings?.map((prod: any) => (
                                   <div key={prod.id} className="flex justify-between text-xs items-center">
                                      <span className="font-bold">{prod.product_name}</span>
                                      <span className="text-slate-500">Rp {(prod.price_offer||0).toLocaleString('id-ID')} / {prod.unit}</span>
                                   </div>
                               ))}
                            </div>
                            <div className="mt-4 pt-4 border-t border-slate-100 flex justify-between items-center">
                               {s.phone ? (
                                  <a href={`https://wa.me/${s.phone.replace(/\D/g,'')}`} target="_blank" rel="noreferrer" className="text-xs bg-emerald-100 hover:bg-emerald-200 text-emerald-800 font-bold py-1.5 px-3 rounded-lg flex items-center gap-1"><PhoneCall size={12}/> Hubungi WA</a>
                               ) : (<span/>)}
                            </div>
                          </div>
                        ))
                    ...
                    <tbody className="divide-y divide-slate-100 text-slate-700">
                      {purchaseRequests.map((pr) => (
                        <tr key={pr.id}>
                          ...
                          <td className="py-3 truncate max-w-[100px]">{pr.product?.productName}</td>
                          <td className="py-3">
                            <span
