...
import { Layers, PlusCircle, Search, Edit2, Trash2, Tag, Check, X, CheckCircle, Save, ImageIcon, Info, Coffee } from 'lucide-react';
...
export function MenuTab({ appConfig, products, rawMaterials, recipes, onRefresh }: MenuTabProps) {
      ...
      const response = await fetch('/api/products', {
        ...
        body: JSON.stringify({
          ...
          recipes: recipeIngredients.length > 0 ? {
            ...
            totalCost: recipeIngredients.reduce((sum, ing) => sum + (ing.amount * (rawMaterials.find(m => m.id === ing.materialId)?.unitCost || 0)), 0)
          } : undefined
        })
      });
                        ...
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="w-12 h-12 rounded-xl overflow-hidden bg-slate-50 border border-slate-200 shadow-xs flex-shrink-0 flex items-center justify-center">
                              {p.imageUrl ? (
                                <img src={p.imageUrl} alt={p.name} className="w-full h-full object-cover"  referrerPolicy="no-referrer" onError={(e) => { e.currentTarget.src = "https://placehold.co/400x400/f8fafc/94a3b8?text=Produk+Tanpa+Foto"; }} />
                              ) : (
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in" id="add-menu-modal">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl w-full max-w-xl p-6 space-y-4">
            ...
            <form onSubmit={handleCreateProduct} className="space-y-3.5 text-xs">
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Catatan Racikan & Instruksi Seduh</label>
                <textarea 
              ...
              <div className="bg-slate-50/70 p-3 rounded-2xl border border-slate-200/60 space-y-2.5">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[9px] font-black bg-indigo-50 text-indigo-700 px-1.5 py-0.5 rounded uppercase tracking-wider font-mono">Bahan Baku POS</span>
                </div>
                ...
                {recipeIngredients.length > 0 ? (
                  <div className="space-y-1 max-h-24 overflow-y-auto pr-1">
                    {recipeIngredients.map((ing, idx) => {
                      ...
                      return (
                        <div key={idx} className="flex justify-between items-center bg-white p-2 border border-slate-100 rounded-lg shadow-2xs">
                          <div className="flex items-center gap-1.5">
                            <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 shrink-0" />
                            <span className="font-bold text-slate-700 text-[11px] truncate max-w-[150px]">{mat?.name || 'Bahan Baku'}</span>
                            <span className="text-[10px] text-slate-400">({ing.amount} {mat?.stockUnit || 'unit'})</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="font-mono font-bold text-slate-600 text-[10px]">Rp {costValue.toLocaleString('id-ID')}</span>
                            <button
                              type="button"
                              onClick={() => {
                  ...
                  <p className="text-[10px] text-slate-400 italic text-center py-2.5 bg-white rounded-lg border border-slate-100">Belum ada bahan baku terhubung. Tambahkan bahan di bawah ini.</p>
                )}
                ...
                <div className="bg-white p-2 rounded-xl border border-slate-100 flex gap-2 items-center">
                  ...
                  >
                    ...
                    {rawMaterials.map((mat) => (
                      <option key={mat.id} value={mat.id}>{mat.name} ({mat.stockUnit})</option>
                    ))}
                  </select>
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <span className="text-[9.5px] text-slate-400 font-bold font-mono min-w-[20px]">
                      {rawMaterials.find(m => m.id === tempMaterialId)?.stockUnit || ''}
                    </span>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                      if (!tempMaterialId) return alert('Pilih bahan baku terlebih dahulu!');
                      if (tempAmount <= 0) return alert('Jumlah pemanduan bahan harus lebih besar dari 0!');
                      const existIdx = recipeIngredients.findIndex(r => r.materialId === tempMaterialId);
                      if (existIdx !== -1) {
                        const updated = [...recipeIngredients];
                        updated[existIdx].amount += tempAmount;
                        setRecipeIngredients(updated);
                      } else {
                        setRecipeIngredients([...recipeIngredients, { materialId: tempMaterialId, amount: tempAmount }]);
                      }
                      setTempMaterialId('');
                      setTempAmount(0);
                    }}
                ...
                {(() => {
                  const calculatedHPP = recipeIngredients.reduce((total, ing) => {
                    const mat = rawMaterials.find(m => m.id === ing.materialId);
                    return total + (mat ? ing.amount * mat.unitCost : 0);
                  }, 0);

                  const marginPct = Number(prodPrice) > 0 
                    ? ((Number(prodPrice) - calculatedHPP) / Number(prodPrice)) * 100 
                    : 0;
                  ...
                  return (
                    <div className="p-2 bg-indigo-50/40 rounded-xl border border-indigo-100 flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-1.5">
                          <span className="text-[10px] text-slate-500 font-bold">HPP Resep:</span>
                          <span className="font-mono font-black text-indigo-700 text-[10.5px]">Rp {calculatedHPP.toLocaleString('id-ID')}</span>
                        </div>
                        <div className="flex items-center gap-1">
                          <span className="text-[9px] text-slate-400 font-medium">Margin Jual:</span>
                          <span className={`text-[10px] font-extrabold ${
                            marginPct >= 70 ? 'text-blue-600' : marginPct >= 40 ? 'text-emerald-600' : 'text-rose-600'
                          }`}>
                            {marginPct.toFixed(1)}% {marginPct >= 70 ? '🟢 (Maksimum)' : marginPct >= 40 ? '🟢 (Sehat)' : '🔴 (Rendah)'}
                          </span>
                        </div>
                      </div>
                      <button
                        type="button"
                        onClick={() => setProdCostPrice(Math.round(calculatedHPP))}
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-3 items-center">
                  ...
                  <div className="h-16 w-full rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-center overflow-hidden relative">
                    {prodImageUrl ? (
                      <>
                        <img src={prodImageUrl} alt="Preview" className="w-full h-full object-cover"  referrerPolicy="no-referrer" onError={(e) => { e.currentTarget.src = "https://placehold.co/400x400/f8fafc/94a3b8?text=Produk+Tanpa+Foto"; }} />
                        <button 
                          type="button" 
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-3 items-center">
                  ...
                  <div className="h-16 w-full rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-center overflow-hidden relative">
                    {prodImageUrl ? (
                      <>
                        <img src={prodImageUrl} alt="Pratinjau Edit" className="w-full h-full object-cover"  referrerPolicy="no-referrer" onError={(e) => { e.currentTarget.src = "https://placehold.co/400x400/f8fafc/94a3b8?text=Produk+Tanpa+Foto"; }} />
                        <button 
                          type="button" 
                          onClick={() => setProdImageUrl('')} 
                          className="absolute inset-0 bg-black/40 text-white flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity text-[9px] font-bold cursor-pointer"
