...
import { Cloud, Sparkles, Globe, Terminal, Info, ExternalLink, AlertTriangle, Database, FileText, Check, Shield, Mail, RefreshCw, X } from 'lucide-react';
...
export function DriveTab({ appConfig, onUpdateConfig, onRefresh }: DriveTabProps) {
  ...
  const addDriveLog = (msg: string) => {
    setDriveLogs(prev => [...prev, `[${new Date().toLocaleTimeString('id-ID')}] ${msg}`]);
  };
  ...
  const confirmOAuthSimulation = () => {
    ...
    logAuditActivity('Admin', 'OAUTH_CONNECT', 'Google Account linked successfully');
  ...
  const handleTestConnection = () => {
    ...
    setTimeout(() => {
      setSyncProgress(80);
      addDriveLog('Mengunggah JSON & Media ke Drive (Folder: ' + driveStoreFolder + ')');
    }, 1200);
    setTimeout(() => {
      setSyncProgress(100);
      setIsSyncing(false);
  ...
  const handleTestScheduleSend = () => {
    ...
    addDriveLog(`Mempersiapkan Laporan via Email (Target: ${recipientEmail})...`);
    setTimeout(() => {
            ...
            <div className="relative flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              ...
              <div className="bg-slate-950/40 px-5 py-4 rounded-2xl border border-indigo-500/20 text-center min-w-[200px]">
                ...
                <span className="text-[9px] text-slate-400 block mt-0.5">
                  {driveConnected ? (appConfig.cashierEmail || easyEmail) : 'Sistem Lokal Aktif'}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-200/50 shadow-sm space-y-4">
              ...
              <div className="space-y-3.5 text-xs">
                ...
                <p className="text-[10px] text-amber-805 leading-relaxed font-sans bg-amber-50/50 p-3 rounded-2xl border border-amber-100/50 text-amber-900">
                  💡 <strong>Integrasi Hibrida:</strong> Server akan membaca database penjualan harian dan stok bahan baku Anda, mengompilasinya menjadi <u>Laporan doc/Excel rapi</u>, mengunggah ke Google Drive dan Sheets secara real-time, lalu mengirimkannya via email.
                </p>
                ...
                <div className="space-y-3.5">
                  ...
                  >
                    <div className="w-8 h-8 rounded-full bg-indigo-600 text-white font-bold flex items-center justify-center text-xs">
                      {((googleConnectMode === 'easy' ? easyEmail : appConfig.cashierEmail) || 'O').substring(0, 1).toUpperCase()}
                    </div>
                    <div className="text-left font-sans">
                      <p className="text-xs font-bold text-slate-800">{googleConnectMode === 'easy' ? (easyEmail ? easyEmail.split('@')[0] : 'Owner') : (appConfig.cashierName || 'Pemilik Toko')}</p>
                      <p className="text-[10px] text-slate-400 font-mono">{googleConnectMode === 'easy' ? (easyEmail || 'owner@ledgerline.my.id') : (appConfig.cashierEmail || `${tenantId}@ledgerline.my.id`)}</p>
                    </div>
                  </div>
