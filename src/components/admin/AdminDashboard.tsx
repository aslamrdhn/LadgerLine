...
export default function AdminDashboard() {
  const [activeTab, setActiveTab] = useState<'overview' | 'tenants' | 'suppliers' | 'audit'>('overview');
  ...
  return (
    <div className="p-6 h-full overflow-y-auto bg-slate-50">
      <div className="flex gap-4 mb-6">
        <button onClick={() => setActiveTab('overview')} className={`px-4 py-2 rounded \${activeTab === 'overview' ? 'bg-indigo-600 text-white' : 'bg-white'}`}>Overview</button>
        <button onClick={() => setActiveTab('tenants')} className={`px-4 py-2 rounded \${activeTab === 'tenants' ? 'bg-indigo-600 text-white' : 'bg-white'}`}>Tenants</button>
        <button onClick={() => setActiveTab('suppliers')} className={`px-4 py-2 rounded \${activeTab === 'suppliers' ? 'bg-indigo-600 text-white' : 'bg-white'}`}>Suppliers</button>
        <button onClick={() => setActiveTab('audit')} className={`px-4 py-2 rounded \${activeTab === 'audit' ? 'bg-indigo-600 text-white' : 'bg-white'}`}>Audit Log</button>
      </div>
