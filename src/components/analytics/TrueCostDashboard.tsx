...
export default function TrueCostDashboard({
  products,
  tenantId,
}: {
  products: any[];
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-2xl font-black text-slate-800">
            True Cost Engine Dashboard
          </h2>
          <p className="text-sm text-slate-500 font-medium">
            Deep analysis of operational allocation & real margin
          </p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {products.map((product) => {
            ...
            return (
              <div
                key={product.id}
                className="bg-white rounded-2xl p-6 border border-slate-100 shadow-xl shadow-slate-200/40 relative overflow-hidden group"
              >
                ...
                <div className="space-y-4 mb-6">
                  <div className="flex justify-between items-center text-sm border-b border-slate-100 pb-2">
                    ...
                    <span className="font-black text-slate-800">
                      Rp {data.sellingPrice.toLocaleString()}
                    </span>
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between items-center text-sm mb-1">
                      ...
                      <span className="font-bold text-rose-600 font-mono">
                        - Rp {Math.round(data.materialCost).toLocaleString()}
                      </span>
                    </div>
                  ...
                  <div>
                    <div className="flex justify-between items-center text-sm mb-1">
                      ...
                      <span className="font-bold text-rose-600 font-mono">
                        - Rp {Math.round(data.operationalCost).toLocaleString()}
                      </span>
                    </div>
                    {data.operationalBreakdown &&
                    data.operationalBreakdown.length > 0 ? (
                      <div className="pl-3 border-l-2 border-slate-100 space-y-1 mt-1">
                        {data.operationalBreakdown.map(
                          (item: any, idx: number) => (
                            <div
                              key={idx}
                              className="flex justify-between items-center text-xs"
                      ...
                      <div className="pl-3 border-l-2 border-slate-100 text-xs text-slate-400 italic">
                        No overhead mapped
                      </div>
                    )}
                  </div>
                  ...
                  {data.tax > 0 && (
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className="font-bold text-amber-600 font-mono">
                        - Rp {Math.round(data.tax).toLocaleString()}
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="pt-4 border-t border-slate-100 mb-4">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <span className="text-sm font-black text-slate-900 font-mono">
                      Rp {Math.round(data.totalTrueCost).toLocaleString()}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-black text-emerald-600 font-mono">
                      Rp {Math.round(data.realProfit).toLocaleString()}
                    </span>
                  </div>
                </div>
                ...
                <div className="bg-slate-50 p-4 rounded-xl flex items-center justify-between border border-slate-100">
                  <div className="flex items-center gap-2">
                    <TrendingUp
                      size={18}
                      className={
                        data.margin > 40 ? "text-emerald-500" : "text-amber-500"
                      }
                    />
                  ...
                  <div
                    className={`text-xl font-black ${data.margin > 40 ? "text-emerald-600" : "text-amber-600"}`}
                  >
