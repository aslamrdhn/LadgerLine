...
import TrueCostDashboard from './TrueCostDashboard';
...
export default function FinancialReports({ financeLogs, products, rawMaterials, orders, appConfig, backupHistory, onRefresh }: FinancialReportsProps) {
  const { uiMode } = useUiMode();
  const [encryptionStatus, setEncryptionStatus] = useState<'IDLE' | 'ENCRYPTING' | 'SECURED'>('SECURED');
  const [selectedBackupFile, setSelectedBackupFile] = useState<File | null>(null);
  const [restoreMessage, setRestoreMessage] = useState<string>('');
  const [activeReportTab, setActiveReportTab] = useState<'pl' | 'neraca' | 'cashflow' | 'truecost'>('pl');
  ...
  const [reportPeriod, setReportPeriod] = useState<'all' | 'daily' | 'weekly' | 'monthly' | 'yearly'>('all');
  ...
  const [enableOpHoursFilter, setEnableOpHoursFilter] = useState<boolean>(false);

  // States untuk Logger lokal Jurnal Kas (Buku Kas)
  const [logType, setLogType] = useState<'income' | 'expense'>('expense');
  const [logCategory, setLogCategory] = useState<string>('Beban Operasional Lainnya');
  ...
  const handleAddlokalFinanceLog = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        setSubmitSuccess('Transaksi lapangan berhasil dicatatkan ke Buku Besar Buku Kas!');
        setLogAmount('');
        setLogDesc('');
        // Trigger data reload
        onRefresh();
      ...
      setSubmitError('Terjadi kesalahan sambungan jaringan ke server database.');
    } finally {
    ...
    try {
      const response = await fetch('/api/finance/log', {
        ...
        body: JSON.stringify({
          ...
          amount: amountToPay,
          description: 'Setoran Pajak SSP/SPTPD Cafe per Bulan ke Rekening Kas Negara (Dirjen Pajak) via POS',
  ...
  const isTimeInOperatingHours = React.useCallback((timeStr: string) => {
  ...
  }, [financeLogs, orders, reportPeriod, selectedDate, selectedMonth, selectedYear, enableOpHoursFilter, isTimeInOperatingHours]);

  const { filteredLogs, filteredOrders, startLocalDate, endLocalDate } = filteredData;
  ...
  const totalFixedStaffWage = React.useMemo(() => uniqueLedgerDates * dailyStaffWage, [uniqueLedgerDates, dailyStaffWage]);
  const totalFixedUtility = React.useMemo(() => uniqueLedgerDates * dailyUtilityCost, [uniqueLedgerDates, dailyUtilityCost]);
  const totalFixedRent = React.useMemo(() => uniqueLedgerDates * dailyRentCost, [uniqueLedgerDates, dailyRentCost]);
  const totalFixedOverhead = React.useMemo(() => totalFixedStaffWage + totalFixedUtility + totalFixedRent, [totalFixedStaffWage, totalFixedUtility, totalFixedRent]);
  ...
  const grossProfit = React.useMemo(() => grossRevenue - computedCOGS, [grossRevenue, computedCOGS]);
  const accountingNetProfit = React.useMemo(() => grossProfit - totalOPEX - totalFixedOverhead, [grossProfit, totalOPEX, totalFixedOverhead]);
  ...
  const downloadCSVReport = () => {
    ...
    link.setAttribute('download', `Laporan_Buku_Kas_${reportPeriod}_${new Date().toISOString().split('T')[0]}.xls`);
  ...
  return (
    <div className="space-y-6" id="reports-tab">
      ...
      <div className="bg-slate-900 text-white p-6 rounded-2xl border border-slate-800 shadow-xl space-y-6" id="operational-filter-panel">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 pb-4 border-b border-slate-800">
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-1">
              Data transaksi, rekap penjualan POS, dan saringan sirkulasi buku kas Buku-Kas menyelaraskan jam operasional kedai kopi Anda.
            </p>
          </div>
          ...
          <div className="md:col-span-6 space-y-4">
            ...
            {reportPeriod === 'all' && (
              <div className="p-4 bg-slate-950 rounded-xl border border-slate-800 text-xs text-slate-400 leading-relaxed">
                Menyajikan akumulasi seluruh data buku kas tanpa batas tanggal kalender. Gunakan saringan menu untuk melihat performa spesifik harian, mingguan, bulanan, atau tahunan. 
              </div>
            )}
            ...
            {(reportPeriod === 'daily' || reportPeriod === 'weekly') && (
              <div className="space-y-2">
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono">
                  {reportPeriod === 'daily' ? 'Pilih Hari Operasional (Tanggal)' : 'Pilih Acuan Tanggal Minggu Terkait'}
                </label>
                ...
                {reportPeriod === 'weekly' && (
                  <div className="text-[10px] bg-indigo-950/40 text-indigo-300 font-mono p-2 rounded-lg border border-indigo-900/40">
                    Siklus Mingguan Aktif: <span className="font-bold text-white">{getWeekRange(selectedDate).start}</span> s/d <span className="font-bold text-white">{getWeekRange(selectedDate).end}</span>
                  </div>
                )}
              </div>
            )}
            ...
            {reportPeriod === 'monthly' && (
              <div className="space-y-2">
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono">Pilih Bulan Pembukuan</label>
                <div className="flex gap-2">
                  <input
                    type="month"
                    value={selectedMonth}
                  ...
                  <button
                    type="button"
                    onClick={() => setSelectedMonth(new Date().toISOString().split('T')[0].substring(0, 7))}
                    className="px-3 bg-slate-800 hover:bg-slate-700 text-slate-200 text-[11px] font-bold rounded-xl transition-all cursor-pointer"
                  >
                    Bulan Ini
                  </button>
            ...
            {reportPeriod === 'yearly' && (
              <div className="space-y-2">
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono">Isi Tahun Anggaran</label>
                <div className="flex gap-2">
                  <select
                    value={selectedYear}
            ...
            {enableOpHoursFilter ? (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[9px] font-bold text-slate-405 text-slate-400 uppercase tracking-widest mb-1 font-mono">Jam Buka Cafe</label>
                    <input
                      type="time"
                      value={openingHour}
                  ...
                  <div>
                    <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1 font-mono">Jam Tutup Cafe</label>
                    <input
                      type="time"
                      value={closingHour}
                ...
                <div className="space-y-1">
                  <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-widest font-mono">Ganti Instan Ke Shift:</label>
                  <div className="flex flex-wrap gap-1">
                    <button
                      type="button"
                      onClick={() => { setOpeningHour('08:00'); setClosingHour('16:00'); }}
                    ...
                    <button
                      type="button"
                      onClick={() => { setOpeningHour('16:00'); setClosingHour('23:00'); }}
                      className="px-2 py-1 bg-slate-800 hover:bg-slate-700 text-slate-100 text-[9px] font-mono rounded-md font-bold transition-all cursor-pointer"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => { setOpeningHour('10:00'); setClosingHour('23:59'); }}
                      className="px-2 py-1 bg-slate-800 hover:bg-slate-700 text-slate-100 text-[9px] font-mono rounded-md font-bold transition-all cursor-pointer"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => { setOpeningHour('18:00'); setClosingHour('03:00'); }}
                      className="px-2 py-1 bg-slate-800 hover:bg-slate-700 text-slate-100 text-[9px] font-mono rounded-md font-bold transition-all cursor-pointer"
                    >
              ...
              <div className="p-4 bg-slate-950 rounded-xl border border-slate-800 text-xs text-slate-500 leading-relaxed font-sans">
                Pengukuran saringan jam dinonaktifkan. Seluruh jam operasi (pagi, sore/malam hingga dini hari) diakumulasi lurus tanpa interupsi waktu.
              </div>
            )}
          </div>
        </div>
        ...
        <div className="pt-4 border-t border-slate-800">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h4 className="text-[11px] font-bold uppercase tracking-wider text-slate-300 font-mono">
              Penjumlahan Totalan Pembukuan Kas Terfilter (Real-time Filtered Sums)
            </h4>
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3.5">
            ...
            <div className="bg-slate-950/60 p-3 rounded-xl border border-slate-800 space-y-1">
              <span className="text-[9px] font-bold uppercase text-slate-500 block font-mono">Gross Revenue (Kotor)</span>
              ...
              <span className="text-[9px] text-slate-400 block">Total kas masuk sebelum diskon/beban.</span>
            </div>

            {/* 2. Diskon */}
            <div className="bg-slate-950/60 p-3 rounded-xl border border-slate-800 space-y-1">
              <span className="text-[9px] font-bold uppercase text-slate-500 block font-mono">Total Diskon</span>
              <span className="text-xs font-bold text-rose-450 text-rose-400 font-mono block">
                Rp {totalDiscountsInput.toLocaleString('id-ID')}
              </span>
              <span className="text-[9px] text-slate-400 block">Jumlah promo Kasir POS dikurangkan.</span>
            </div>

            {/* 3. Estimasi HPP */}
            <div className="bg-slate-950/60 p-3 rounded-xl border border-slate-800 space-y-1">
              <span className="text-[9px] font-bold uppercase text-slate-500 block font-mono">Estimasi HPP (COGS)</span>
              <span className="text-xs font-bold text-slate-205 text-slate-100 font-mono block">
                Rp {computedCOGS.toLocaleString('id-ID')}
              </span>
              <span className="text-[9px] text-slate-400 block">Biaya modal resep menu terjual.</span>
            </div>

            {/* 4. Total OPEX & Overhead */}
            <div className="bg-slate-950/60 p-3 rounded-xl border border-slate-800 space-y-1">
              <span className="text-[9px] font-bold uppercase text-slate-500 block font-mono">OPEX & Overhead</span>
              ...
              <span className="text-[9px] text-slate-400 block">Mutasi bahan terbuang + beban flat ruko.</span>
            </div>
            ...
            <div className="bg-slate-950/60 p-3 rounded-xl border border-slate-800 space-y-1 col-span-2 lg:col-span-1">
              <span className="text-[9px] font-bold uppercase text-slate-500 block font-mono">True Net Profit (Bersih)</span>
              <span className={`text-sm font-black font-mono block ${accountingNetProfit >= 0 ? 'text-blue-400' : 'text-rose-500'}`}>
                Rp {accountingNetProfit.toLocaleString('id-ID')}
              </span>
              <span className="text-[9px] text-slate-400 block">Kelayakan laba final Buku-Kas berlisensi.</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex border border-slate-200 p-1 bg-slate-50 rounded-xl gap-2 font-sans overflow-x-auto" id="reporting-type-tabbar">
        <button
          type="button"
      ...
      {activeReportTab === 'pl' && (
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-xs space-y-5 animate-fade-in" id="accounting-pl-ledger-card">
          ...
          <div className="p-3.5 bg-[#EEF2F6] rounded-xl border border-slate-200 flex items-start gap-2.5 text-[11px] text-slate-700">
            ...
            <div className="space-y-1">
              ...
              <p className="leading-relaxed text-slate-500">
                Buku kas ini menghindari kesalahan ganda: modal bahan mentak yang dibeli (restock CAPEX) <span className="font-bold">tidak dikurangkan langsung secara mentah</span> dari laba bersih usaha. Yang dikurangkan adalah <span className="font-bold">HPP Bahan Terpakai (COGS)</span> sesuai porsi resep produk yang terjual ditambah <span className="font-bold">Beban Susutan / Waste terbuang</span> secara real-time.
              </p>
            </div>
          </div>
        </div>
      ...
      {activeReportTab === 'truecost' && (
         <TrueCostDashboard products={products} tenantId={appConfig?.tenantId || 'aslam-brew'} />
      )}
      
      {/* RENDER TAB 2: NERACA Buku Kas */}
      {activeReportTab === 'neraca' && (() => {
        const initialLaci = appConfig.cashRegisterFund !== undefined ? appConfig.cashRegisterFund : 500000;
        const totalInflow = financeLogs.filter(l => l.type === 'income').reduce((sum, l) => sum + l.amount, 0);
        const totalOutflow = financeLogs.filter(l => l.type === 'expense').reduce((sum, l) => sum + l.amount, 0);
        const computedCash = initialLaci + totalInflow - totalOutflow;

        const inventoryBahanBakuValue = (rawMaterials || []).reduce((sum, mat) => sum + (mat.stockQuantity * (mat.unitCost || 0)), 0);
        const inventoryMenuJadiValue = (products || []).reduce((sum, prod) => sum + (prod.stock * (prod.costPrice || 1000)), 0);
        
        const totalAktiva = computedCash + inventoryBahanBakuValue + inventoryMenuJadiValue;
        
        const totalPajakPenjualan = orders.reduce((sum, ord) => sum + (ord.tax || 0), 0);
        const setoranPajakValue = financeLogs
          .filter(log => log.type === 'expense' && log.description && typeof log.description === 'string' && log.description.includes('Setoran Pajak'))
          .reduce((sum, log) => sum + log.amount, 0);
        const utangPajakValue = Math.max(0, totalPajakPenjualan - setoranPajakValue);

        const modalDisetorValue = appConfig.initialCapital !== undefined ? appConfig.initialCapital : 15000000;
        const labaBerjalan = accountingNetProfit;
        const modalPenyesuaian = totalAktiva - utangPajakValue - modalDisetorValue - labaBerjalan;
        const totalEquityCombined = modalDisetorValue + labaBerjalan + modalPenyesuaian;
        const totalPasiva = utangPajakValue + totalEquityCombined;

        return (
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-xs space-y-6 animate-fade-in" id="accounting-neraca-card">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-100 pb-4">
              <div>
                <h3 className="text-base font-bold text-slate-800">Laporan Neraca Buku Kas (Balance Sheet Statement)</h3>
                <p className="text-[11px] text-slate-400 mt-0.5">Memotret posisi keuangan, nilai aset persediaan stok, laba ditahan, dan modal usaha secara real-time.</p>
              </div>
              <span className="text-[10px] uppercase font-bold px-3 py-1 rounded bg-[#EBF5FF] text-blue-800 border border-blue-100 font-mono tracking-wider flex items-center gap-1">
                <ShieldCheck size={12} className="text-blue-600 animate-pulse" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden bg-slate-50/50">
                <div className="bg-slate-100 border-b border-slate-200 p-3 text-xs font-bold uppercase text-slate-700 flex justify-between items-center font-sans">
                  ...
                  <span className="text-[10px] text-indigo-600 font-mono font-extrabold">DEBET</span>
                </div>
                <div className="divide-y divide-slate-100 text-xs font-sans">
                  <div className="p-3 flex justify-between items-center bg-white">
                    <div>
                      <p className="font-semibold text-slate-800">1-1000 Kas dan Setara Kas</p>
                      <p className="text-[10px] text-slate-400 mt-0.5">Kas laci POS & Kas Bank (Dana Awal + Penjualan - OPEX - Belanja)</p>
                    </div>
                    <span className="font-mono font-bold text-slate-700 font-sans">Rp {computedCash.toLocaleString('id-ID')}</span>
                  </div>
                  <div className="p-3 flex justify-between items-center bg-white">
                    <div>
                      <p className="font-semibold text-slate-800">1-2100 Persediaan Bahan Baku</p>
                      <p className="text-[10px] text-slate-400 mt-0.5">Nilai perolehan stok bahan mentah (biji kopi, susu, sirup, cup)</p>
                    </div>
                    <span className="font-mono font-bold text-slate-700 font-sans">Rp {inventoryBahanBakuValue.toLocaleString('id-ID')}</span>
                  </div>
                  <div className="p-3 flex justify-between items-center bg-white">
                    <div>
                      <p className="font-semibold text-slate-800">1-2200 Persediaan Menu Jadi</p>
                      <p className="text-[10px] text-slate-400 mt-0.5">Nilai perolehan stok botol minuman dingin & kue siap saji</p>
                    </div>
                    <span className="font-mono font-bold text-slate-700 font-sans">Rp {inventoryMenuJadiValue.toLocaleString('id-ID')}</span>
                  </div>
                </div>
                <div className="p-4 bg-indigo-50 border-t border-slate-200 flex justify-between items-center font-bold text-sm">
                  <span className="text-indigo-900 font-extrabold uppercase">TOTAL JUMLAH AKTIVA (A)</span>
                  <span className="font-mono text-indigo-700">Rp {totalAktiva.toLocaleString('id-ID')}</span>
                </div>
              </div>
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden bg-slate-50/50">
                <div className="bg-slate-100 border-b border-slate-200 p-3 text-xs font-bold uppercase text-slate-700 flex justify-between items-center font-sans">
                  ...
                  <span className="text-[10px] text-blue-600 font-mono font-extrabold">KREDIT</span>
                </div>
                <div className="divide-y divide-slate-100 text-xs font-sans">
                  ...
                  <div className="p-3 flex justify-between items-center bg-white">
                    <div className="space-y-1">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <p className="font-semibold text-slate-800">2-1100 Utang Pajak Penjualan</p>
                      ...
                      <p className="text-[10px] text-slate-400 leading-normal">
                        Kewajiban pajak yang dipungut dari struk konsumen untuk disetorkan ke Kas Negara.
                        {setoranPajakValue > 0 && <span className="text-emerald-600 block font-semibold font-sans">Telah disetor: Rp {setoranPajakValue.toLocaleString('id-ID')}</span>}
                      </p>
                      {utangPajakValue > 0 && (
                        <button
                          type="button"
                        ...
                        >
                          {payingTax ? 'Memproses SSP...' : 'Setor Pajak Sekarang (SSP)'}
                        </button>
                      )}
                    </div>
                    <span className="font-mono font-bold text-slate-700 shrink-0">Rp {utangPajakValue.toLocaleString('id-ID')}</span>
                  </div>
                  ...
                  <div className="p-3 flex justify-between items-center bg-white">
                    <div>
                      <p className="font-semibold text-slate-800">3-1100 Modal Disetor Awal (Owner Capital)</p>
                      <p className="text-[10px] text-slate-400 mt-0.5">Nilai investasi kapital pertama yang dideklarasikan pemilik kedai</p>
                    </div>
                    <span className="font-mono font-bold text-slate-700">Rp {modalDisetorValue.toLocaleString('id-ID')}</span>
                  </div>
                  <div className="p-3 flex justify-between items-center bg-white">
                    <div>
                      <p className="font-semibold text-slate-800">3-1200 Saldo Laba Ditahan Komprehensif</p>
                      <p className="text-[10px] text-slate-400 mt-0.5">Akumulasi keuntungan bersih harian yang diputar kembali ke bisnis</p>
                    </div>
                    <span className="font-mono font-bold text-emerald-600">Rp {labaBerjalan.toLocaleString('id-ID')}</span>
                  </div>
                  <div className="p-3 flex justify-between items-center bg-white">
                    <div>
                      <p className="font-semibold text-slate-800">3-1300 Koreksi Penyesuaian Modal Lancar Awal</p>
                      <p className="text-[10px] text-slate-400 mt-0.5">Penyesuaian valuasi aset riil (laci kasir + bahan baku + menu) terhadap modal luar</p>
                    </div>
                    <span className="font-mono font-bold text-indigo-400">Rp {modalPenyesuaian.toLocaleString('id-ID')}</span>
                  </div>
                </div>
                <div className="p-4 bg-indigo-50 border-t border-slate-200 flex justify-between items-center font-bold text-sm">
                  <span className="text-indigo-900 font-extrabold uppercase">TOTAL JUMLAH PASIVA (B)</span>
                  <span className="font-mono text-indigo-700">Rp {totalPasiva.toLocaleString('id-ID')}</span>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 bg-emerald-50 border border-emerald-100 text-emerald-800 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              <div className="flex items-center gap-2.5 text-emerald-800 font-sans">
                <div className="w-8 h-8 rounded-full bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center shrink-0">
                  <CheckCircle2 size={16} className="text-emerald-605 text-emerald-600 animate-bounce" />
                </div>
                <div>
                  <p className="font-extrabold text-xs text-emerald-950">NERACA SEIMBANG SECARA MATEMATIS AKTIF (BALANCED✓)</p>
                  <p className="text-[10.5px] text-emerald-700 mt-0.5 animate-pulse">Persamaan akuntansi dasar <strong className="font-mono">Aktiva = Liabilitas + Ekuitas (Rp {totalAktiva.toLocaleString('id-ID')} = Rp {totalPasiva.toLocaleString('id-ID')})</strong> terpenuhi.</p>
                </div>
              </div>
      ...
      {activeReportTab === 'cashflow' && (() => {
        ...
        const netOperatingCashflow = grossRevenue - computedCOGS - opexCashPaidOut;
        const netFinancingCashflow = initialLaci + (appConfig.initialCapital || 15000000);

        return (
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-xs space-y-5 animate-fade-in" id="accounting-cashflow-card">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-100 pb-4 min-h-[50px]">
              <div>
                <h3 className="text-base font-bold text-slate-800">Laporan Arus Kas (Statement of Cash Flows)</h3>
                <p className="text-[11px] text-slate-400 mt-0.5">Menunjukkan bagaimana kas kasir bergerak masuk dan keluar melalui kegiatan operasional, investasi, dan ketersediaan modal.</p>
              </div>
              <span className="text-[10px] uppercase font-bold px-3 py-1 rounded bg-amber-50 text-amber-700 border border-amber-100 font-mono flex items-center gap-1">
                <TrendingUp size={12} className="text-amber-500 animate-pulse" />
            ...
            <div className="overflow-hidden border border-slate-200 rounded-xl bg-[#FCFCFD]">
              ...
              <div className="divide-y divide-slate-100 font-sans text-xs">
                ...
                <div className="grid grid-cols-12 p-3 bg-white">
                  <div className="col-span-8 text-slate-600 pl-4">Penerimaan Kas Klien dari Penjualan POS</div>
                  <div className="col-span-4 text-right font-mono text-emerald-600 font-bold">+Rp {grossRevenue.toLocaleString('id-ID')}</div>
                </div>
                <div className="grid grid-cols-12 p-3 bg-white">
                  <div className="col-span-8 text-slate-600 pl-4">Pembayaran Kas untuk COGS / Resep Terjual</div>
                  <div className="col-span-4 text-right font-mono text-rose-600 font-bold">-Rp {computedCOGS.toLocaleString('id-ID')}</div>
                </div>
                <div className="grid grid-cols-12 p-3 bg-white">
                  <div className="col-span-8 text-slate-600 pl-4">Pembayaran Kas untuk Beban Tetap Toko (Gaji, Sewa, Utilitas)</div>
                  <div className="col-span-4 text-right font-mono text-rose-600 font-bold">-Rp {totalFixedOverhead.toLocaleString('id-ID')}</div>
                </div>
                <div className="grid grid-cols-12 p-3 bg-white">
                  <div className="col-span-8 text-slate-600 pl-4">Pembayaran Kas untuk Beban Operasional Lainnya (Suspensi / Waste)</div>
                  <div className="col-span-4 text-right font-mono text-rose-600 font-bold">-Rp {totalOPEX.toLocaleString('id-ID')}</div>
                </div>
                <div className="grid grid-cols-12 p-2.5 bg-slate-50/70 font-bold border-t border-b">
                  <div className="col-span-8 pl-4 text-slate-700">Arus Kas Bersih dari Aktivitas Operasional Kerja</div>
                  <div className="col-span-4 text-right font-mono text-indigo-650 text-indigo-600">
                    {netOperatingCashflow >= 0 ? '+' : ''}Rp {netOperatingCashflow.toLocaleString('id-ID')}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-12 p-3 bg-white">
                  <div className="col-span-8 text-slate-600 pl-4">Setoran Modal Utama oleh Pemilik Toko</div>
                  <div className="col-span-4 text-right font-mono text-emerald-600 font-bold">+Rp {(appConfig.initialCapital || 15000000).toLocaleString('id-ID')}</div>
                </div>
                <div className="grid grid-cols-12 p-3 bg-white">
                  <div className="col-span-8 text-slate-600 pl-4">Kebutuhan Float Dana Laci Kasir Awal</div>
                  <div className="col-span-4 text-right font-mono text-emerald-600 font-bold">+Rp {initialLaci.toLocaleString('id-ID')}</div>
                </div>
                <div className="grid grid-cols-12 p-2.5 bg-slate-50/70 font-bold border-t border-b">
                  <div className="col-span-8 pl-4 text-slate-700">Arus Kas Bersih dari Aktivitas Pendanaan</div>
                  <div className="col-span-4 text-right font-mono text-indigo-650 text-indigo-600">+Rp {netFinancingCashflow.toLocaleString('id-ID')}</div>
                </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-150 shadow-xs p-6 space-y-4" id="lokal-ledger-logger-form">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5 font-sans">Catatkan pengeluaran harian (belanja es, utilitas, bonus barista) atau pemasukan lainnya agar laporan laba rugi & arus kas sinkron real-time.</p>
          </div>
        ...
        <form onSubmit={handleAddlokalFinanceLog} className="grid grid-cols-1 md:grid-cols-12 gap-4 font-sans">
          {/* Tipe Transaksi */}
          <div className="md:col-span-3 space-y-1.5">
            <label className="block text-xs font-semibold text-slate-600">Jenis Aliran</label>
          ...
          {/* Kategori Akuntansi */}
          <div className="md:col-span-3 space-y-1.5">
            <label className="block text-xs font-semibold text-slate-600">Kategori Akuntansi Buku Kas</label>
            <select
              value={logCategory}
            ...
            >
              {logType === 'expense' ? (
                <>
                  <option value="Beban Bahan Baku">Beban Bahan Baku (Restok/Gudang)</option>
                  <option value="Beban Gaji & Upah">Beban Gaji & Upah Barista</option>
                  <option value="Beban Utilitas (Air/Listrik/Wifi)">Beban Utilitas (Air/Listrik/Wifi)</option>
                  <option value="Beban Sewa Tempat">Beban Sewa Tempat</option>
                  <option value="Beban Pemeliharaan & Kerusakan">Beban Pemeliharaan/Mesin Kopi</option>
                  <option value="Beban Operasional Lainnya">Beban Operasional Lainnya</option>
                </>
              ) : (
                <>
                  <option value="Pendapatan Non-Operasional">Pendapatan Non-Operasional (Bunga, Hibah dll)</option>
                  <option value="Setoran Modal Tambahan">Setoran Modal Tambahan (Owner)</option>
          ...
          {/* Nominal Rp */}
          <div className="md:col-span-3 space-y-1.5">
            <label className="block text-xs font-semibold text-slate-600">Nominal Transaksi (Rupiah Rp)</label>
          ...
          {/* Tanggal */}
          <div className="md:col-span-3 space-y-1.5">
            <label className="block text-xs font-semibold text-slate-600">Tanggal Pencatatan</label>
            <input
              type="date"
              value={logDate}
          ...
          <div className="md:col-span-9 space-y-1.5">
            <label className="block text-xs font-semibold text-slate-600">Keterangan Deskripsi Rinci</label>
            <input
              type="text"
        ...
        <div className="overflow-x-auto text-xs">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 font-sans">
              {financeLogs.slice().reverse().slice(0, 100).map((log) => (
          ...
          <div className="space-y-3">
            ...
            <div className="p-3.5 bg-white/5 rounded-xl border border-white/10 space-y-2 font-mono text-[10px]">
              ...
              <p className="text-[9px] text-indigo-300 leading-normal border-t border-white/5 pt-1.5 mt-1.5">
                Mengamankan pembukuan harian backend cloud tersentralisasi dari akses ilegal.
              </p>
            </div>
          </div>
