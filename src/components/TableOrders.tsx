...
export default function TableOrders({ tables, products, orders, appConfig, onRefresh }: TableOrdersProps) {
  // UI Panels and controls
  const [selectedTable, setSelectedTable] = useState<any>(null);
  const [activeSegment, setActiveSegment] = useState<'ordering' | 'kds'>('ordering');
  ...
  const pendingOrders = useMemo(() => {
    ...
    return orders.filter(o => !o.prepStatus || o.prepStatus === 'Pending' || o.prepStatus === 'Waiting Payment').slice(-50);
  }, [orders]);
  ...
  const servicedOrders = useMemo(() => {
    return orders.filter(o => o.prepStatus === 'Ready' || o.prepStatus === 'Completed').slice(-30); // limit ready/completed to save Ram
  }, [orders]);
  ...
  const activeSimulatedOrder = useMemo(() => {
    ...
    return orders.find(o => o.id === customerSuccessOrder.id) || customerSuccessOrder;
  ...
  const handleBulkGenerateTables = async () => {
    ...
    if (!confirm(`Apakah Anda yakin ingin melakukan bulk-regenerate ${count} meja baru? Seluruh meja lama milik tenant Anda akan dihapus.`)) {
      return;
    }
  ...
  const handleKdsStatusChange = async (orderId: string, nextPrepStatus: string) => {
    try {
      await fetch(`/api/orders/${orderId}/status`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'X-Tenant-Id': tenantId },
  ...
  return (
    <div className="space-y-6 text-slate-800 font-sans" id="ledgerline-smarttable-evolution">
      ...
      <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 inline-flex gap-1 shadow-2xs">
        ...
        >
          ...
          {orders.filter(o => !o.prepStatus || o.prepStatus === 'Pending' || o.prepStatus === 'Preparing' || o.prepStatus === 'Waiting Payment').length > 0 && (
            <span className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-rose-500 font-mono text-[9px] font-black text-white animate-bounce">
              {orders.filter(o => !o.prepStatus || o.prepStatus === 'Pending' || o.prepStatus === 'Preparing' || o.prepStatus === 'Waiting Payment').length}
            </span>
          )}
        </button>
      </div>
      ...
      {activeSegment === 'ordering' ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-5 bg-white p-6 rounded-2xl border border-slate-100 shadow-xs space-y-6">
            <div className="border-b border-slate-100 pb-4">
              ...
              <p className="text-xs text-slate-500 mt-1">
                Generate token lunas unik, kelola status meja terisi, serta unduh label tag barcode modern per meja.
              </p>
            </div>
            ...
            <div className="bg-slate-50/55 p-4 rounded-xl border border-slate-150 space-y-3">
              ...
              <div className="flex gap-2 items-center">
                <div className="relative flex-1">
                  ...
                  />
                  <span className="absolute right-2.5 top-2.5 text-[10px] uppercase font-bold text-slate-400">Meja</span>
                </div>
                <button
                  type="button"
              ...
              <p className="text-[9.5px] text-slate-400 leading-normal">
                ⚠️ Men-generate meja baru otomatis menghasilkan string token unik untuk melumpuhkan fraud manipulasi harga produk di sisi browser.
              </p>
            </div>
                ...
                {tables.map(table => (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center w-full mt-3">
                      <span className="text-[9.5px] font-mono text-slate-400 font-medium">#{table.publicToken || table.id}</span>
                      <span className={`text-[8.5px] font-bold uppercase px-1.5 py-0.5 rounded-sm ${
                        selectedTable && selectedTable.id === table.id 
            ...
            {selectedTable && (
              <div className="p-4 bg-[#FDFBF7] rounded-xl border border-amber-100 space-y-4 shadow-sm" id="manager-table-summary">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="font-extrabold text-amber-950 text-sm">{selectedTable.name}</h3>
                    <p className="text-[10.5px] text-slate-500 font-mono">Token: {selectedTable.publicToken || 'EMPTY_TOKEN'}</p>
                  </div>
                  <button
                ...
                <div className="border-t border-dashed border-amber-200/50 pt-3 space-y-2 text-xs">
                  <div className="flex justify-between">
                    <span className="text-slate-400">Endpoint URL (Secure Link):</span>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-slate-700 font-semibold">{appConfig.storePhone || '0812-4455-6677'}</span>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="lg:col-span-7 bg-white p-8 rounded-3xl border border-slate-200 shadow-sm relative flex flex-col justify-center min-h-[660px]" id="customer-live-qr-panel">
            {selectedTable ? (
              <div className="text-center space-y-6">
                ...
                <div className="space-y-2">
                  <h3 className="text-xl font-black text-slate-800">Scan untuk Pesan di {selectedTable.name}</h3>
                  <p className="text-sm text-slate-500 max-w-sm mx-auto">
                    Pelanggan dapat memindai barcode ini langsung dari kamera HP mereka untuk membuka menu digital tanpa saveeksport aplikasi.
                  </p>
                </div>
                <div className="bg-amber-50 p-4 rounded-xl border border-amber-200 max-w-sm mx-auto text-left break-all">
                  <p className="text-[10px] font-black uppercase text-amber-800 mb-1 tracking-wider">Tautan Live Akses Publik (URL):</p>
                  <a href={`/m/${selectedTable.publicToken}`} target="_blank" rel="noreferrer" className="text-xs font-mono text-amber-700 hover:underline">
              ...
              <div className="text-center text-slate-500 space-y-4">
                ...
                <p>Pilih meja di sebelah kiri untuk melihat QR Code dan Link pemesanan Live.</p>
              </div>
            )}
          </div>
        ...
        <div className="space-y-6 text-slate-800 animate-fade-in" id="kds-kanban-board">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200 text-xs text-left">
            <div className="bg-white p-3.5 rounded-xl border border-slate-150 flex flex-col justify-between shadow-xs">
              <span className="text-[10px] uppercase font-black text-slate-400">Total KDS Antrean</span>
              <span className="text-lg font-mono font-black text-slate-800 mt-1">{orders.length} Tiket</span>
            </div>
            <div className="bg-white p-3.5 rounded-xl border border-slate-150 flex flex-col justify-between shadow-xs">
              <span className="text-[10px] uppercase font-black text-[#D97706]">Belum Lunas (Pending)</span>
              <span className="text-lg font-mono font-black text-[#D97706] mt-1">
                {orders.filter(o => o.paymentStatus === 'Pending' || o.paymentStatus === 'Waiting Payment').length} Antrean
              </span>
            </div>
            <div className="bg-white p-3.5 rounded-xl border border-slate-150 flex flex-col justify-between shadow-xs">
              <span className="text-[10px] uppercase font-black text-amber-800">Diseduh (Preparing)</span>
              <span className="text-lg font-mono font-black text-amber-800 mt-1">
                {preparingOrders.length} Gelas
              </span>
            </div>
            <div className="bg-white p-3.5 rounded-xl border border-slate-150 flex flex-col justify-between shadow-xs">
              <span className="text-[10px] uppercase font-black text-emerald-600">Siap Saji (Ready)</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {pendingOrders.length > 0 ? (
                <div className="space-y-4 max-h-[500px] overflow-y-auto pr-1">
                  {pendingOrders.map((order) => {
                    const isUnpaid = order.paymentStatus === 'Pending' || order.paymentStatus === 'Waiting Payment';
                    const isSmartTable = order.notes && typeof order.notes === 'string' && order.notes.includes('[Smart Table');

                    return (
                      <div key={order.id} className="bg-white p-4.5 rounded-xl border border-slate-200 shadow-sm space-y-3.5">
                        <div className="flex justify-between items-start">
                          <div className="text-left">
                            ...
                            <h4 className="font-mono font-black text-slate-800 text-[10.5px] mt-2">{order.id}</h4>
                          </div>
                          <span className="text-[10px] text-slate-450 font-bold font-mono">
                            {new Date(order.orderTime).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}
                          </span>
                        </div>
                        ...
                        {isUnpaid ? (
                          ...
                          >
                            <Check size={11} />
                            {isConfirmingPayment === order.id ? 'Mengonfirmasi...' : '💸 Konfirmasi Bayar Lunas'}
                          </button>
                        ) : (
                          <button
                            type="button"
                            onClick={() => handleKdsStatusChange(order.id, 'Preparing')}
                ...
                <p className="text-[11.5px] text-slate-400 italic text-center py-8">Dapur santai. Tidak ada antrean minuman masuk.</p>
              )}
            </div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4.5 space-y-4">
              ...
              {preparingOrders.length > 0 ? (
                ...
                <p className="text-[11.5px] text-slate-400 italic text-center py-8">Belum ada portafilter dipasang di bawah gilingan kopi.</p>
              )}
            </div>
              ...
              {servicedOrders.length > 0 ? (
                <div className="space-y-4 max-h-[500px] overflow-y-auto pr-1">
                  {servicedOrders.map((order) => (
                    <div key={order.id} className={`p-4.5 rounded-xl border shadow-sm space-y-3.5 transition-colors ${
                      order.prepStatus === 'Completed' ? 'bg-emerald-50/20 border-emerald-100 opacity-60' : 'bg-white border-slate-200'
                    }`}>
                      <div className="flex justify-between items-start">
                        <div className="text-left">
                          <span className={`text-[9.5px] font-black uppercase ${
                            order.prepStatus === 'Completed' ? 'text-slate-400' : 'text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded'
                          }`}>
                            Meja: {order.tableNumber || 'Kasir'} • {order.prepStatus === 'Completed' ? 'Selesai' : 'Siap Saji'}
                          </span>
                          <h4 className="font-mono font-black text-slate-800 text-[10.5px] mt-2">{order.id}</h4>
                        </div>
                        <span className="text-[10px] text-slate-450 font-bold font-mono">
                      ...
                      <div className="border-t border-dashed border-slate-150 pt-2.5 space-y-1.5 text-slate-700 text-xs text-left font-semibold">
                        {order.items.map((item: any, i: number) => {
                          ...
                          return (
                            <div key={i} className="text-[11px]">
                              • {item.quantity}x {p?.name || 'Item'}
                              {item.notes && <span className="block text-[10px] text-slate-500 italic pl-3 leading-normal font-sans">({item.notes})</span>}
                            </div>
                          );
                        })}
                      </div>
                      ...
                      {order.prepStatus !== 'Completed' ? (
                        <button
                          type="button"
                          onClick={() => handleKdsStatusChange(order.id, 'Completed')}
                          className="w-full py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold text-[10px] uppercase rounded-lg cursor-pointer transition-all active:scale-95 text-center flex items-center justify-center gap-1 shadow-2xs"
                        >
                ...
                <p className="text-[11.5px] text-slate-400 italic text-center py-8">Belum ada gelas kosong yang siap dihantar.</p>
              )}
            </div>
      ...
      <div className="col-span-1 md:col-span-12 bg-slate-50 p-6 rounded-2xl border border-slate-200/85 space-y-4 shadow-3xs" id="table-barcode-info">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-200 pb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">Bimbingan tata kelola QR code meja, pembayaran BYO QRIS, serta fungsionalitas pengamanan margin.</p>
            </div>
          </div>
          <button
        ...
        {showTableHelp && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 animate-fade-in text-xs font-sans font-semibold text-slate-600 leading-relaxed">
            <div className="bg-white p-4.5 rounded-xl border border-slate-150 space-y-2">
              <span className="font-extrabold text-amber-950">📱 1. Filosofi Bring Your Own QRIS</span>
              <p className="text-[11px] leading-relaxed">
                Anda tidak perlu membayar biaya transfer per transaksi e-payment ke perbankan atau integrator pihak ketiga. Cukup tempel label stiker QRIS milik Anda (Bank Mandiri, BCA, ShopeePay, GPN, dll.) di laci konfig, pelanggan Smart Table memindai m-banking mereka seperti biasa, dan kasir barista Anda mengonfirmasi lunas di KDS sekali klik.
              </p>
            </div>
            <div className="bg-white p-4.5 rounded-xl border border-slate-150 space-y-2">
              <span className="font-extrabold text-amber-950">🛡️ 2. Pengamanan Manipulasi Harga (Security)</span>
              <p className="text-[11px] leading-relaxed">
                Pembeli tidak bisa menyabotase database harga dari browser inspect element. Sistem kami menetapkan string tokens acak 8 karakter lurus ke database, mengabaikan input harga palsu dari frontend, mendata ulang subtotal dari record murni server, dan melacak detail pembayaran secara ketat di Security Audit Trail.
              </p>
            </div>
            <div className="bg-white p-4.5 rounded-xl border border-slate-150 space-y-2">
              <span className="font-extrabold text-amber-950">📦 3. Proteksi Pengurangan Stok Buku-Besar</span>
              <p className="text-[11px] leading-relaxed">
                 Bahan baku penting seperti biji kopi Arabika Sumatra Gayo dan Susu Greenfields tidak akan terpotong secara konyol ketika pengunjung baru sekadar iseng mengirim pesanan. Depresiasi stok dan kalkulasi HPP harian hanya disusutkan secara tegas ketika kasir Anda menekan tombol lunas konfirmasi bayar.
              </p>
            </div>
          </div>
        )}
      ...
      {showPrintModal && selectedTable && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in" id="print-tag-dialog-modal">
          <div className="bg-white max-w-sm w-full rounded-2xl border border-slate-200 overflow-hidden shadow-2xl flex flex-col justify-between">
            <div className="p-6 text-center space-y-5" id="printable-area-tag-coffee">
              ...
              <div className="border-[5px] border-amber-950/20 p-5 rounded-xl bg-[#FAF9F5] space-y-5 relative">
                ...
                <div className="bg-white/80 backdrop-blur-xs p-3.5 border border-amber-100 rounded-lg text-left text-[9px] text-[#543A20] leading-snug space-y-1">
                  <p className="font-extrabold uppercase border-b border-amber-150 pb-1 mb-1 tracking-wider text-amber-950">💡 LANGKAH E-ORDER MANDIRI:</p>
