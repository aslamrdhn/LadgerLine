...
function SystemClock() {
  ...
  React.useEffect(() => {
    const update = () => {
      const now = new Date();
      setTime(now.toLocaleString('id-ID', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }) + ' WIB');
    };
    update();
  ...
  return <p className="text-sm font-mono font-medium text-slate-700">{time || 'Loading...'}</p>;
}

export default function Dashboard({ products, rawMaterials, financeLogs, appConfig, onRefresh, onNavigate }: DashboardProps) {
  const [aiInsight, setAiInsight] = useState<string>('');
  const [loadingAi, setLoadingAi] = useState<boolean>(false);
  const [showGlossary, setShowGlossary] = useState<boolean>(true);
  const [activeGlossaryTab, setActiveGlossaryTab] = useState<'income' | 'expense' | 'net_profit' | 'profit_margin' | 'stock_warning' | 'gemini_ai'>('income');
  const [detailedGlossaryModal, setDetailedGlossaryModal] = useState<boolean>(false);
  ...
  const handleCopyPODraft = (id: string, itemName: string, warningLimit: number, currentStock: number, unit: string, supplierName: string) => {
    const draftText = 
...
Kami ingin memesan kembali bahan baku tersebut di atas. Mohon infokan penawaran harga & ketersediaan pengiriman. Terima kasih.`;
  ...
  const netProfit = React.useMemo(() => totalIncome - totalExpense, [totalIncome, totalExpense]);
  const marginPercentage = React.useMemo(() => totalIncome > 0 ? Math.round((netProfit / totalIncome) * 100) : 0, [totalIncome, netProfit]);

  // Deteksi bahan baku kritis & produk kritis - DIOPTIMALKAN DENGAN MEMOISASI
  const lowMaterialsAll = React.useMemo(() => rawMaterials.filter(m => m.stockQuantity <= m.warningLimit), [rawMaterials]);
  const lowProductsAll = React.useMemo(() => products.filter(p => p.stock <= p.warningLimit), [products]);
  const lowMaterials = React.useMemo(() => lowMaterialsAll.slice(0, 5), [lowMaterialsAll]);
  const lowProducts = React.useMemo(() => lowProductsAll.slice(0, 5), [lowProductsAll]);
  const totalWarnings = React.useMemo(() => lowMaterialsAll.length + lowProductsAll.length, [lowMaterialsAll, lowProductsAll]);
  ...
  return (
    <div className="space-y-6" id="dashboard-tab">
              ...
              {lowMaterials.map((mat) => (
                <div key={mat.id} className="p-4 bg-amber-50/50 rounded-xl border border-amber-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-sm">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => handleCopyPODraft(mat.id, mat.name, mat.warningLimit, mat.stockQuantity, mat.stockUnit, mat.supplierName)}
                    ...
                    >
                      {copiedItemId === mat.id ? 'Tersalin !' : 'Salin Draft PO'}
                    </button>
                    <a 
              ...
              {lowProducts.map((prod) => (
                <div key={prod.id} className="p-4 bg-slate-50 rounded-xl border border-slate-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-sm">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => handleCopyPODraft(prod.id, prod.name, prod.warningLimit, prod.stock, 'Porsi', prod.supplierName)}
                      className={`px-3 py-2 text-xs font-bold font-sans rounded-lg transition-all cursor-pointer border ${
                    ...
                    >
                      {copiedItemId === prod.id ? 'Tersalin !' : 'Salin Draft PO'}
                    </button>
                    <a 
      ...
      <div className="bg-gradient-to-br from-slate-50 to-indigo-50/20 p-6 rounded-2xl border border-slate-200 shadow-2xs space-y-4" id="mkt-intelligence-dashboard-hook">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div className="space-y-1 text-left">
            ...
            <h3 className="text-base font-extrabold text-slate-900">Tren Harga Pokok Komoditas Regional (Yogyakarta & DIY)</h3>
            <p className="text-xs text-slate-500">Hasil pooling transaksi riil dienkripsi dari ratusan kafe mitra Ledger Line.</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-left font-sans">
          {[
            { label: 'House Blend (per Kg)', avg: 'Rp 215.000', trend: 'Stabil', col: 'text-slate-650 bg-slate-50', bg: 'bg-white' },
            { label: 'Fresh Milk (per Ltr)', avg: 'Rp 17.200', trend: 'Naik +4.5%', col: 'text-rose-600 bg-rose-50 font-bold', bg: 'bg-white' },
            { label: 'Gula Aren (per Ltr)', avg: 'Rp 24.500', trend: 'Stabil', col: 'text-slate-650 bg-slate-50', bg: 'bg-white' },
            { label: 'Premium Matcha (per Kg)', avg: 'Rp 730.000', trend: 'Turun -1.2%', col: 'text-emerald-600 bg-emerald-50 font-bold', bg: 'bg-white' }
          ].map((c, i) => (
            <div key={i} className={`${c.bg} p-4 rounded-xl border border-slate-200 shadow-2xs space-y-1.5`}>
              <p className="text-[10px] uppercase font-bold text-slate-400 font-mono tracking-tight leading-none">{c.label}</p>
              <p className="text-[15.5px] font-mono font-black text-slate-800 tracking-tight leading-none mt-1">{c.avg}</p>
              <span className={`inline-block text-[9px] px-1.5 py-0.5 rounded-md ${c.col} mt-1`}>
                Indeks {c.trend}
              </span>
            </div>
          ))}
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-xs space-y-4" id="menu-profitability-block">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Analisis HPP (Cost of Goods Sold), harga jual, dan rasio margin laba bersih produk aktif untuk keputusan harga ideal.</p>
          </div>
          ...
          <table className="w-full text-left border-collapse text-xs font-sans">
            <thead className="bg-slate-50 border-b border-slate-150 font-semibold text-slate-500 font-mono text-[10px] uppercase">
              <tr>
                ...
                <th className="p-3 text-center">Status Tier & Rekomendasi Pricing</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 bg-white">
              {(() => {
                const filtered = products.filter(p => {
                  const matchSearch = p.name.toLowerCase().includes(profitSearch.toLowerCase());
                  const matchCat = selectedProfitCat === 'All' || p.category === selectedProfitCat;
                ...
                return filtered.map((p) => {
                  ...
                  const marginPct = p.price > 0 ? Math.round((profitVal / p.price) * 100) : 0;
                  ...
                  if (marginPct >= 65) {
                    ...
                    tierColor = 'bg-emerald-50 text-emerald-700 border-emerald-100';
                    recText = 'Sangat profitable! Pertahankan kualitas bahan baku tetap premium.';
                  } else if (marginPct < 40) {
                    tierLabel = 'Margin Rendah';
                    tierColor = 'bg-rose-50 text-rose-700 border-rose-100';
                    const idealPrice = Math.ceil((p.costPrice * 2) / 1000) * 1000;
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="p-3 font-semibold text-slate-800">
                        <div className="flex items-center gap-2">
                          <span className={`w-2.5 h-2.5 rounded-full ${marginPct < 40 ? 'bg-rose-500 animate-pulse' : marginPct >= 65 ? 'bg-emerald-500' : 'bg-blue-500'}`} />
                          {p.name}
                        </div>
                      </td>
                      <td className="p-3 text-slate-500">{p.category}</td>
                      <td className="p-3 text-right font-mono text-slate-600">Rp {p.costPrice.toLocaleString('id-ID')}</td>
                      <td className="p-3 text-right font-mono font-bold text-slate-800">Rp {p.price.toLocaleString('id-ID')}</td>
                      <td className="p-3 text-right font-mono font-bold text-slate-800">Rp {profitVal.toLocaleString('id-ID')}</td>
                      <td className="p-3 text-right">
                        <span className={`font-mono font-black ${marginPct >= 65 ? 'text-emerald-600' : marginPct < 40 ? 'text-rose-600 font-extrabold' : 'text-blue-600'}`}>
                          {marginPct}%
                        </span>
                      </td>
                      <td className="p-3 font-sans">
                        <div className="flex flex-col md:flex-row items-start md:items-center gap-2">
                          ...
                          <span className="text-[10.5px] text-slate-400 italic leading-tight">{recText}</span>
                        </div>
                      </td>
                    </tr>
                  );
      ...
      <div className="bg-slate-50/60 p-6 rounded-2xl border border-slate-250/80 space-y-5 shadow-xs" id="dashboard-help-glossary">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-200/60 pb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400">Navigasi cepat istilah finansial, ambang kritis operasional, dan mesin asisten kecerdasan buatan Gemini AI.</p>
            </div>
          </div>
          <button
                ...
                return (
                  ...
                  >
                    <div className="space-y-2">
                      ...
                      <div>
                        <h4 className="text-xs font-black text-slate-800 group-hover:text-indigo-600 transition-colors leading-tight">{card.title}</h4>
                        <p className="text-[9px] text-slate-400 font-mono tracking-tight">{card.subtitle}</p>
                      </div>
                    </div>
                    <div className="border-t border-slate-100 pt-2.5 mt-2 flex justify-between items-center w-full">
                      <span className="text-[10px] font-mono font-extrabold text-slate-700 leading-none">{card.metric}</span>
                      <span className="text-[8px] text-indigo-500 font-bold tracking-widest font-sans uppercase">TINJAU</span>
                    </div>
                  </button>
                );
              })}
        ...
        {detailedGlossaryModal && (
          <div className="fixed inset-0 bg-slate-950/75 backdrop-blur-xs flex items-center justify-center p-4 z-100 animate-fade-in" style={{ zIndex: 9999 }} id="detailed-glossary-overlay">
            <div className="bg-white rounded-3xl w-full max-w-xl p-6 sm:p-8 shadow-2xl border border-slate-200 relative animate-scale-in flex flex-col gap-5 overflow-hidden text-slate-700">
              ...
              <div className="flex justify-between items-start border-b border-slate-100 pb-3 mt-2">
                <div>
                  <span className="text-[9px] uppercase font-mono tracking-widest text-[#4f46e5] font-black leading-none">Console Metrik Buku-Kas</span>
                  <h3 className="text-base font-black text-slate-900 tracking-tight mt-1 flex items-center gap-1.5 uppercase">
                    📘 Detail Metrik: {
                      activeGlossaryTab === 'income' ? 'Total Pemasukan' :
                      activeGlossaryTab === 'expense' ? 'Total Pengeluaran' :
                      activeGlossaryTab === 'net_profit' ? 'Laba Bersih Akhir' :
                      activeGlossaryTab === 'profit_margin' ? 'Rasio Profit Margin' :
                      activeGlossaryTab === 'stock_warning' ? 'Sistem Peringatan Stok' :
              ...
              <div className="space-y-4 text-xs font-sans">
                {activeGlossaryTab === 'income' && (
                  <div className="space-y-3.5">
                    <p className="text-slate-500 leading-relaxed font-semibold">
                      Akumulasi seluruh uang yang masuk dari transaksi penjualan di kasir POS. Angka ini mencakup semua jenis metode pembayaran (Tunai, QRIS Dinamis, Kartu Debit, dan Integrasi Midtrans) yang telah tuntas terbayar oleh konsumen sebelum dikurangi pajak daerah PB1 (10%) maupun harga pokok bahan baku (HPP).
                    </p>
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 font-mono text-[11px] space-y-1">
                      <p className="text-slate-400 text-[9px] uppercase font-bold tracking-wider font-sans leading-none">Formula Pencatatan Buku-Kas:</p>
                      <p className="font-extrabold text-slate-800 mt-1">∑ (Porsi Jual × Jumlah Terjual) = Pendapatan Kotor</p>
                    </div>
                    <div className="bg-emerald-50/50 p-3.5 rounded-2xl border border-emerald-100 flex justify-between items-center">
                      <div className="space-y-0.5">
                        ...
                        <span className="text-[10px] text-emerald-800 font-extrabold uppercase tracking-wider">Pemasukan Riil Saat Ini</span>
                      </div>
                      <strong className="text-sm font-mono font-black text-emerald-600">Rp {totalIncome.toLocaleString('id-ID')}</strong>
                    </div>
                  </div>
                )}
                ...
                {activeGlossaryTab === 'expense' && (
                  <div className="space-y-3.5">
                    <p className="text-slate-500 leading-relaxed font-semibold">
                      Akumulasi seluruh biaya operasional ruko yang dicatat secara berkala (seperti sewa lokasi, internet ruko, bonus barista, es batu harian), ditambah dengan nominal pembelanjaan / restock suplai bahan baku kopi dari supplier yang terdaftar pada sistem.
                    </p>
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 font-mono text-[11px] space-y-1">
                      <p className="text-slate-400 text-[9px] uppercase font-bold tracking-wider font-sans leading-none">Formula Komponen Pengeluaran:</p>
                      <p className="font-extrabold text-slate-800 mt-1">Kas Belanja Bahan Baku + Biaya Operasional / Utilitas</p>
                    </div>
                    <div className="bg-rose-50/50 p-3.5 rounded-2xl border border-rose-100 flex justify-between items-center">
                      <div className="space-y-0.5">
                        ...
                        <span className="text-[10px] text-rose-800 font-extrabold uppercase tracking-wider">Pengeluaran Riil Saat Ini</span>
                      </div>
                      <strong className="text-sm font-mono font-black text-rose-600">Rp {totalExpense.toLocaleString('id-ID')}</strong>
                    </div>
                  </div>
                )}
                ...
                {activeGlossaryTab === 'net_profit' && (
                  <div className="space-y-3.5">
                    <p className="text-slate-500 leading-relaxed font-semibold">
                      Keuntungan bersih aktual yang dibawa pulang oleh pemilik kedai (Owner). Jika nilainya positif, tandanya kafe Anda sehat finansial; jika negatif, tandanya kedai Anda mengalami kebocoran anggaran harian. Dihitung langsung dari sirkulasi pendapatan dikurangi belanja bahan baku ruko & pengeluaran utilitas flat.
                    </p>
                    <div className="bg-slate-900 text-white p-4 rounded-2xl border border-slate-800 space-y-2 font-mono text-[10.5px]">
                      <div className="flex justify-between items-center text-slate-450 text-[8.5px] uppercase font-bold tracking-widest font-sans border-b border-slate-800 pb-1.5 leading-none">
                        ...
                        <span className={netProfit >= 0 ? "text-emerald-400" : "text-rose-450"}>
                          {netProfit >= 0 ? "● STATUS SEHAT" : "● RISIKO RUGI"}
                        </span>
                      </div>
                      <p className="text-slate-400">Laba Bersih = Pemasukan - Pengeluaran</p>
                ...
                {activeGlossaryTab === 'profit_margin' && (
                  <div className="space-y-3.5">
                    <p className="text-slate-500 leading-relaxed font-semibold">
                      Persentase efisiensi usaha dalam menghasilkan laba dari setiap rupiah pendapatan. Dihitung otomatis dengan membagi Laba Bersih dengan Total Pemasukan kemudian dikalikan 100. Semakin tinggi rasionya, semakin kuat pertahanan finansial ruko kedai Anda menghadapi inflasi bahan baku.
                    </p>
                    <div className="bg-slate-900 text-white p-4 rounded-2xl border border-slate-800 space-y-2 font-mono text-[10.5px]">
                      <div className="flex justify-between items-center text-slate-405 text-[8.5px] uppercase font-bold tracking-widest font-sans border-b border-slate-800 pb-1.5 leading-none">
                        ...
                        <span className="text-indigo-400 font-sans font-bold">
                          {marginPercentage >= 40 ? "IDEAL MARGIN (>40%)" : "MARGIN TERTANAM"}
                        </span>
                      </div>
                      <p className="text-slate-400">Profit Margin = (Laba Bersih / Total Pemasukan) × 100%</p>
                      <p className="font-extrabold text-white tracking-wider border-t border-slate-850 pt-2 flex flex-wrap gap-1 leading-none mt-1">
                        ...
                        <span className="text-indigo-400 font-black">{marginPercentage}%</span>
                      </p>
                    </div>
                  </div>
                )}
                ...
                {activeGlossaryTab === 'stock_warning' && (
                  <div className="space-y-3.5">
                    <p className="text-slate-500 leading-relaxed font-semibold">
                      Modul cerdas yang memantau sisa stok susu, cup paper, gula cair, atau biji kopi di bawah batas kritis aman (warning limit) yang telah disetel. Jika terlewati, sistem akan menyalakan alarm merah dan menyiapkan tombol salin Purchase Order otomatis untuk supplier.
                    </p>
                    <div className="bg-amber-50 rounded-2xl border border-amber-200 p-4 flex items-center gap-3.5">
                      ...
                      <div className="space-y-0.5 text-xs text-amber-950 font-semibold">
                        <strong className="block text-amber-900 font-bold text-xs uppercase tracking-wide">Ringkasan Darurat Stok Riil</strong>
                        <span className="text-amber-850 text-[11px] leading-relaxed block font-medium mt-0.5">
                          Terdapat <strong className="font-bold text-rose-700 font-mono">{lowMaterials.length} bahan baku</strong> dan <strong className="font-bold text-rose-700 font-mono">{lowProducts.length} produk jadi</strong> berada di bawah batas kritis.
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                {activeGlossaryTab === 'gemini_ai' && (
                  <div className="space-y-3.5">
                    <p className="text-slate-500 leading-relaxed font-semibold">
                      Sebelah modul stok rill adalah asisten audit otomatis yang ditenagai oleh model <strong className="text-violet-600 font-bold">Google Gemini AI</strong>. Menekan tombol "Hasilkan Rekomendasi" akan secara instan membaca kondisi riil pengeluaran, HPP, margin keuntungan, dan restock supplier, memberikan taktik terarah tanpa perhitungan lokal.
                    </p>
                    <div className="bg-slate-900 text-white p-4 rounded-xl border border-slate-800 flex justify-between items-center text-xs font-mono">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="font-bold text-white text-[11.5px]">Google Gemini SDK Integration</p>
                          <p className="text-[8px] text-slate-400 uppercase font-black tracking-wider leading-none mt-0.5">Server-Side Security Shield Shield Active</p>
                        </div>
                      </div>
                      <span className="text-indigo-400 font-extrabold text-[10px]">READY (1.5 Flash)</span>
                    </div>
                  </div>
                )}
              </div>
