...
export default function StorefrontProfile({ appConfig, onUpdateConfig, products, rawMaterials, recipes, onRefresh }: StorefrontProfileProps) {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'profil' | 'menu' | 'tema' | 'swot' | 'drive' | 'karyawan' | 'printer' | 'migration'>('profil');
  ...
  const [easyEmail, setEasyEmail] = useState<string>(() => appConfig.cashierEmail || '');
  const [scheduleEnabled, setScheduleEnabled] = useState<boolean>(true);
  const [scheduleTime, setScheduleTime] = useState<string>('23:00');
  const [recipientEmail, setRecipientEmail] = useState<string>(() => appConfig.cashierEmail || '');
  ...
  return (
    <div className="space-y-6" id="settings-master-container">
      ...
      <div className="bg-slate-950 p-4 rounded-2xl flex flex-col md:flex-row items-start md:items-center justify-between border border-slate-800 shadow-md">
         <div>
            <h3 className="text-white font-bold text-sm">Mode Tampilan Aplikasi</h3>
            <p className="text-xs text-slate-400 mt-1">Gunakan Mode Lanjutan untuk fitur ekstra seperti Simulator, Audit Kripto, atau Midtrans.</p>
         </div>
         <label className="relative inline-flex items-center cursor-pointer mt-4 md:mt-0">
           <input type="checkbox" className="sr-only peer" checked={uiMode === 'advanced'} onChange={(e) => {
           ...
           <span className="ml-3 text-sm font-bold text-white uppercase tracking-wider">{uiMode === 'advanced' ? 'Lanjutan' : 'Sederhana'}</span>
         </label>
      </div>
      ...
      {showAdvancedConfirm && (
         <div className="fixed inset-0 z-[100] bg-slate-950/80 backdrop-blur-xs flex justify-center items-center p-4">
            <div className="bg-slate-900 border border-slate-700 p-6 rounded-2xl max-w-sm w-full shadow-2xl relative animate-fade-in text-center space-y-4">
               ...
               <h3 className="text-lg font-bold text-white">Nyalakan Mode Lanjutan?</h3>
               <p className="text-xs text-slate-400">Mode lanjutan akan menampilkan data developer seperti Terminal Kripto, Midtrans Simulator, dan Panel Database. Pastikan Anda paham teknik dasarnya.</p>
               <div className="flex gap-3 justify-center pt-2">
                  <button onClick={() => setShowAdvancedConfirm(false)} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded-lg text-xs">Batal</button>
                  <button onClick={() => { toggleUIMode('advanced'); setShowAdvancedConfirm(false); }} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-lg text-xs shadow-md shadow-indigo-900/40">Ya, Lanjutkan</button>
               </div>
            </div>
         </div>
      )}
      ...
      {activeSubTab === 'profil' && <ProfileTab appConfig={appConfig} onUpdateConfig={onUpdateConfig} />}
      {activeSubTab === 'menu' && <MenuTab appConfig={appConfig} products={products} rawMaterials={rawMaterials} recipes={recipes} onRefresh={onRefresh} />}

      {activeSubTab === 'tema' && <ThemeTab appConfig={appConfig} onUpdateConfig={onUpdateConfig} />}
      ...
      {activeSubTab === 'drive' && <DriveTab appConfig={appConfig} onUpdateConfig={onUpdateConfig} onRefresh={onRefresh} />}
      {activeSubTab === 'karyawan' && <EmployeesTab appConfig={appConfig} />}
      {activeSubTab === 'printer' && <PrinterTab appConfig={appConfig} onUpdateConfig={onUpdateConfig} />}
